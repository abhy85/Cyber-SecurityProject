...
export default function BlockVisualizer({ ciphertext, current, valid }) {
  const blocks = useMemo(() => splitBlocks(ciphertext), [ciphertext]);

  // ensure valid is always an array
  const validList = useMemo(() => {
    if (!valid) return [];
    return Array.isArray(valid) ? valid : [valid];
  }, [valid]);
  ...
  return (
    <div className="bg-white border border-[#e5e7eb] rounded-xl p-5">
      ...
      {blocks.map((block, bi) => {
        const bytes = Object.values(block);
          ...
          <div key={bi} className="mb-5">
            ...
            <div className="grid grid-cols-16 gap-1">
              {bytes.map((byte, i) => {
                ...
                const isRecovered = validList.some(
                  (v) =>
                    v.block - 1 === bi &&
                    v.byte === i &&
                    v.status === "valid"
